...
const FindEmployees = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/users/search/students', {
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <select value={selectedSkill} onChange={(e) => setSelectedSkill(e.target.value)} style={{ padding: '8px' }}>
        ...
        <select value={experienceLevel} onChange={(e) => setExperienceLevel(e.target.value)} style={{ padding: '8px' }}>
      ...
      <div>
        {students.length === 0 ? (
          ...
          students.map((student) => (
            <div key={student.id} style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ddd', borderRadius: '8px' }}>
              <img
